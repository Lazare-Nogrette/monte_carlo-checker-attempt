...
class CheckerModel:
	...
	def move_piece(self, selected_piece_position, move_position, to_maximize=False):
	...
	def ia_move(self, model, depth_minimax, to_maximize=False):
		if model == "random":
			selected_piece_position, move_position = self.random_model_predict()
			self.move_piece(selected_piece_position, move_position)
			
		elif model == "minimax":
			best_selected_piece_position, best_move_position = self.minimax_model_predict(depth=depth_minimax, to_maximize=to_maximize)
			self.move_piece(best_selected_piece_position, best_move_position)

		if model == "monte_carlo":
			best_selected_piece_position, best_move_position = self.monte_carlo_move(depth=depth_minimax, to_maximize=to_maximize)
			if best_selected_piece_position != None and best_move_position != None:
				self.validate_and_move(best_selected_piece_position, best_move_position)
			else: 
	...
	def minimax_model_predict(self, depth, to_maximize):
		best_score = float("inf") if not to_maximize else -float("inf")
		...
		for possible_action in possible_actions:
			...
			score = self.minimax(robot_turn=False, depth=depth, to_maximize=to_maximize)
			if to_maximize:
				if score >= best_score:
					best_score = score 
	...
	def minimax(self, robot_turn, depth, to_maximize, alpha=-float('inf'), beta=float('inf')):
		game_state = self.check_game_state()
		if game_state == "draw_game":
			return 0
			...
			if depth == 0: 
				...
				for possible_action in possible_actions:
					self.move_piece(*possible_action)					
					score = self.minimax(robot_turn=not robot_turn, depth=depth-1, to_maximize=to_maximize, alpha=alpha, beta=beta)
					self.undo_last_action()
					if to_maximize:
					...
					if to_maximize:
						...
						if robot_turn:
							if alpha >= best_score: #pas sur ici, essayer de switcher les arguments si ça fonctionne pas
								break
							beta = min(beta, best_score)
						else:
							if beta <= best_score:
	...
	def monte_carlo_move(self, depth=30, to_maximize=False):
		possible_moves = self.get_possible_moves() 
		game_state = self.check_game_state()
		if not possible_moves:
			return None
		move_scores = {}
		for selected_piece_position, moves_list in possible_moves.items():
			for move in moves_list:
				final_position = move.get_final_position()
				action_key = (selected_piece_position, final_position)
				move_scores[action_key] = 0

		for action_key in move_scores.keys():
			for _ in range(depth):
				original_state = copy.deepcopy(self.checker_grid)
				original_turn = self.turn  # Précaution pour que l'algorithme se souvienne de quel joueur il est
				self.dict_of_possible_moves = self.get_possible_moves()  # Actualise la liste des move possibles avant la simulation
				if action_key[0] in self.dict_of_possible_moves:
					self.move_piece(*action_key)
					result = self.evaluate_grid(self.checker_grid)
					if to_maximize:
						move_scores[action_key] += result if self.turn == 1 else -result
					else:
						move_scores[action_key] += -result if self.turn == 1 else result
					self.checker_grid = copy.deepcopy(original_state)  # Restaure le plateau initial
					self.turn = original_turn # précaution citée ci-dessus

		best_action = max(move_scores, key=move_scores.get, default=None)
		return best_action

	def validate_and_move(self, selected_piece_position, move_position, to_maximize=False):
		self.history.append(copy.deepcopy(self.checker_grid))
		...
		if selected_piece_position in self.dict_of_possible_moves:
			for possible_move_object in self.dict_of_possible_moves[selected_piece_position]:
				if possible_move_object.get_final_position() == move_position:
					# Handle captures
					for piece_position_to_attack in possible_move_object.list_attacked_enemy_pieces:
						row_to_attack, col_to_attack = piece_position_to_attack
						self.checker_grid[row_to_attack][col_to_attack] = 0
