...
for _ in tqdm.tqdm(range(number_games_to_test)):
	...
	while True:
		checker_model_object.ia_move(model="random", depth_minimax=5, to_maximize=False)
		...
		checker_model_object.ia_move(model="monte_carlo", depth_minimax=30, to_maximize=True)
		game_state = checker_model_object.check_game_state()
		if game_state == "draw_game":
			break
